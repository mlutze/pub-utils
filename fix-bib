...
class LookupCitationTransformer(BlockMiddleware):
    ...
    def transform_entry(self, entry: Entry, library: Library) -> Block:
        if is_done(entry):
            return entry
        doi_field = entry.fields_dict.get("doi", None)
        if doi_field is None:
            log("Cannot look up citation for DOIless entry: " + entry.key)
            return entry
        else:
            doi = doi_field.value
            try:
                crossref_bibtex = cn.content_negotiation(doi)
            ...
            try:
                ...
                log("error parsing crossref entry for entry: " + entry.key + " with doi: " + doi)
                return entry
